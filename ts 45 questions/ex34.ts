/*Pizza:Think of at least three kinds of your favorite pizza.Store these pizza names in a array,and then use a for loop to print 
the name of each pizza.
.Modify your for loop to print a sentence using the name of the pizza instead of printing just the name of pizza.
.Add a line at the end of your program,outside the for loop that states how much you like pizza.The output should consist three or more lines
about the kinds of pizza yoou like and then an additional line at the end such as I really love pizza.*/
//Define the array of favorite pizzas
const favoritePizzas=['Peproni','BBQ Chicken','Margharita'];
//print the name of each pizza using a for loop
console.log("My favorite pizzas are:");
for(let i=0; i<favoritePizzas.length;i++){
    console.log(favoritePizzas[i]);
}
//print a sentence for each pizza
console.log("\nI like these pizzas:");
for(let i=0; i<favoritePizzas.length;i++){
    console.log("I like${favoritePizzas[i]}+Pizza");
}
//print an additional sentence about how much I like pizza
console.log('\nI really love pizza!');
